...
d_orig = pd.read_csv('outputs/processed.csv')
filter_outlier = True
do_plots = False
...
def proc_plots(d, j, do_plots = True):
    # carpetas
    pth = f'figs/by_observer/{j}'
    ...
    if do_plots:
        ...
        ax.legend()
        fig.savefig(f'{pth}/{j}_histogram_conic_cilindrical.png', dpi = 350)
        #plt.show()
        ...
        ax.set_ylabel('Cantidad de eventos')
        ax.legend()
        fig.savefig(f'{pth}/{j}_scatter_conic_cilindrical.png', dpi = 350)
        #plt.show()
        ...
        fig.savefig(f'{pth}/{j}_comparacion_pluvios.png', dpi = 350)
        #plt.show()
    ...
    fig.savefig(f'{pth}/{j}_qqplots.png', dpi = 300)
    #plt.show()


if filter_outlier:
    d_orig = d_orig[(d_orig.d_pluv > -10) & (d_orig.d_conic > -10)]

for j in d_orig.observador.unique():
    #print(j)
    d = d_orig[d_orig.observador == j]
    proc_plots(d, j)
